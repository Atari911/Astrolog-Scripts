...
---------------------------------------------------------------
JupiterGenerator.sh (file-in) (year) (length) (file-out)

Usage: 
------------------------------------------------------------------
This script will generate all of the Jupiter tranits for 
...
the Jupiter transits for is the third arg. Then finally enter the
file that you would like to output.\n
Example: 
------------------------------------------------------------------
./JupiterGenerator.sh ~/Astrology/Jason.Spohn/Info.Jason 2016 10 file\n
This will print the Jupiter transits to the natal listed for the
year of 2016 on forward for 10 years. The file named 'file' will be
created with only the Jupiter transits listed.\n
Notes:
------------------------------------------------------------------
...
>&2; exit 1
	...
	printf "\nJupiterGenerator.sh help   <-For usage info!\n\n"
fi
...
grep -i jupiter /tmp/astrolog.script > $OUTPUT_FILE

sed -i '/Jupiter$/d' $OUTPUT_FILE

sed -i "1i ####################################################################\n" $OUTPUT_FILE
sed -i "1i Start year: $DATE_YEAR Length: $DATE_LENGTH year" $OUTPUT_FILE
sed -i "1i Jupiter Transits for $INFO_FILE" $OUTPUT_FILE
